/*
	DEVELOP TEAM:
		DESIGN BY FAINA SAMAROVA (http://vk.com/id232842027 | faina.arsenovna@mail.ru);
		DEVELOP BY TAMIK LOKYAEV (http://Tamik.ru | hello@Tamik.ru).
...
$(document).ready(function() {
	...
	if(device.mobile()) {
		$("#compositionEl").click(function(event) {
			event.preventDefault();
			if($("#compositionEl").hasClass("open")) {
				$("#compositionEl").removeClass("open");
				$(".composition").slideUp(250);
			}
			else {
				$("#compositionEl").addClass("open");
				$(".composition").slideDown(250);
			}
		});
		$("#goodtoknowEl").click(function() {
			event.preventDefault();
			if($("#goodtoknowEl").hasClass("open")) {
				$("#goodtoknowEl").removeClass("open");
				$(".goodtoknow").slideUp(250);
			}
			else {
				$("#goodtoknowEl").addClass("open");
				$(".goodtoknow").slideDown(250);
			}
		});
		$("#aboutusEl").click(function() {
			event.preventDefault();
			if($("#aboutusEl").hasClass("open")) {
				$("#aboutusEl").removeClass("open");
				$(".aboutus").slideUp(250);
			}
			else {
				$("#aboutusEl").addClass("open");
				$(".aboutus").slideDown(250);
			}
		});
	...
	localStorage.setItem("cost", 1299);
	localStorage.setItem("promocost", 0);
	localStorage.setItem("size", 220);
});
...
function changeSize(value) {
	if(value == 110) {
		_this = $(".size#sizeSmall");
		_that = $(".size#sizeLarge");

		if(_this.hasClass("active") != true) {
			_this.addClass("active");
			_that.removeClass("active");
			localStorage.setItem("cost", 699);
			localStorage.setItem("size", 110);
			checkCost();
		}
	}
	else if(value == 220) {
		_this = $(".size#sizeLarge");
		_that = $(".size#sizeSmall");
...
function checkCost() {
	_this = $("#costValue");
	var cost = (localStorage.getItem("cost") * $("input[name=amount]").val()) * (1 - localStorage.getItem("promocost"));
	_cost = parseInt(cost);
	_this.text(_cost);

	if(_this.text().length < 4) {
		$(".cost").css({"margin-top":"-30px"});
	}
	else
	{
		$(".cost").css({"margin-top":"-70px"});
	}
}
	...
	$.ajax({
		...
		success: function(data) {
			if(data == "true 5") {
				...
				_promocost = parseInt(promocost)
				$("#costValue").text(_promocost);
...
function pay() {
	...
	$.ajax({
		url: "order.php",
		type: "GET",
		data: "form=" + $("input[name=form]").val() + "&device=" + deviceData + "&deviceos=" + deviceOS + "&firstname=" + $("input[name=firstname]").val() + "&lastname=" + $("input[name=lastname]").val() + "&telephone=" + $("input[name=telephone]").val() + "&email=" + $("input[name=email]").val() + "&amount=" + $("input[name=amount]").val() + "&address=" + $("input[name=address]").val() + "&size=" + localStorage.getItem("size") + "&delivery=" + $("input[name=delivery]:checked").val() + "&promo=" + $("input[name=promo]").val().toUpperCase(),
		error: function() {
